export async function createAudioBuffer(
  ...
  try {
    ...
    if (typeof audioData === "string") {
      // Allow normal HTTP caching. Public presets send long-lived Cache-Control
      // from /objects/*; forcing no-store here previously re-downloaded ~700KB
      // on every soundscape switch.
      ...
      console.log("Audio fetch response:", {
        ...
        fromCache: response.headers.get("cf-cache-status") || response.headers.get("x-cache") || undefined,
      });

      if (!response.ok) {
        throw new Error(
          `Failed to fetch audio from ${audioData}: ${response.status} ${response.statusText}`
        );
      }
      ...
      if (!isValidContentType) {
        ...
        throw new Error(
          `Expected audio file but received: ${contentType || "unknown content type"}`
        );
      }
...
export function generateNotificationSound(audioContext: AudioContext): AudioBuffer {
  ...
  for (let i = 0; i < length; i++) {
    ...
    data[i] =
      envelope *
      (Math.sin(2 * Math.PI * freq1 * t) * 0.3 + Math.sin(2 * Math.PI * freq2 * t) * 0.2);
  }
